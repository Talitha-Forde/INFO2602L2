...
class User(db.Model):
    ...
    def add_todo_category(self, todo_id, category):
          ...
          try:
            existing_category = Category.query.filter_by(user_id = self.id, text = category)
            if not existing_category:
              existing_category = Category(self.id, category)
              db.session.add(existing_category)
              db.session.commit()

            user_todo = Todo.query.filter_by(user_id = self_id, id = todo_id).first()
            if not user_todo:
              return False
...
class Todo(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    user_id = db.Column(db.Integer, db.ForeignKey('user.id'), nullable=False)  # Set user_id as a foreign key to user.id
